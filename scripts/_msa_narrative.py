...
def build_narrative(data, ranks=None):
    ...
    if val is not None:
        if val > 10:
            v = f"The Georgia Economics valuation model places {short} <strong>well above</strong> its income-implied fair value ({_pct(val, 0)})"
        elif val > 3:
            v = f"The Georgia Economics valuation model places {short} <strong>modestly above</strong> income-implied fair value ({_pct(val, 0)})"
        elif val >= -3:
            v = f"The Georgia Economics valuation model places {short} <strong>on its income-implied fair value</strong> ({val:.0f}% deviation), so prices track local income fundamentals rather than running ahead of them"
        elif val >= -10:
            v = f"The Georgia Economics valuation model places {short} <strong>modestly below</strong> income-implied fair value ({_pct(val, 0)})"
        else:
            v = f"The Georgia Economics valuation model places {short} <strong>well below</strong> income-implied fair value ({_pct(val, 0)})"
        house.append(v)
    ...
    if aff is not None:
        if aff >= 100:
            house.append(f"Affordability sits above breakeven: the Georgia Economics affordability index reads <strong>{aff:.0f}</strong> "
                         "(Housing Affordability) &mdash; a reading over 100 means the median household can qualify for the median home")
        else:
            house.append(f"Affordability sits below breakeven: the Georgia Economics affordability index reads <strong>{aff:.0f}</strong> "
    ...
    if cr.get("grade") and (cr.get("outlook") in (None, "Positive", "Stable")):
        ...
        s = f"an Georgia Economics credit score of {cr['grade']}"
        if outlook:
    ...
    <section class="analysis">
      <h2>Analysis<span class="as-of-stamp">{stamp} &middot; Generated by Georgia Economics from the data shown on this page</span></h2>

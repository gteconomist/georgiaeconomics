...
| 27 | Exports (by product & destination) | ITA Metropolitan Area Export Data (api.trade.gov/v3/maed via api.data.gov key) | Annual | No |
...
- `ITA_API_KEY` — api.data.gov key for ITA Metropolitan Area Export Data. Free at https://api.data.gov/signup/. Used by `pull_ita.py` to fetch MSA-level exports by NAICS3 product and destination country.
- `IRS_SOI` — no key needed, but pin a curl-able URL for the latest county-to-county migration ZIP (annual release)

**No API key needed for:** FHFA HPI (via FRED with existing key), Census BPS Metro permits (public download), IRS SOI migration (public ZIP). MSA-level export data needs ITA_API_KEY — earlier note assumed Census USA Trade Online covers this, but Census only publishes state and port breakdowns; MSA detail lives on ITA.
